...
- `parse_prime_payments_page()` classifies Membership Central's own error page (a 200 served in place of the widget, observed when the service refuses the library's client from a datacenter address while a browser on the same account gets the payments) as `page_type="error"` instead of raising "Amazon changed the HTML", and `get_prime_payments()` raises a distinct `AmazonOrdersError` for it that points at the parse-from-string path.
