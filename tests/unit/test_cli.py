...
class TestCli(UnitTestCase):
    ...
    @responses.activate
    def test_rewards_balance_command_output_json(self):
        # GIVEN
        self.given_unauthenticated_home_page()
        self.given_login_responses_success()
        with open(os.path.join(self.RESOURCES_DIR, "rewards", "rewards-card-member-two-cards.html"), "r",
                  encoding="utf-8") as f:
            responses.add(
                responses.GET,
                f"{self.test_config.constants.REWARDS_CARD_URL}",
